...
export default function ResetPassword() {
  ...
  const formik = useFormik({
    ...
    [apiError, setApiError] = useState<string | null>(),
    [loading, setLoading] = useState<boolean>(false)

  function handleReset(values: object) {
    console.log(values);
    setLoading(true)
    axios
      .get(
      ...
      .catch((failResponse) => {
        ...
        setApiError("This account doesn't exist");
      })
      .finally(() =>{
        setLoading(false)
      })
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-100 px-4">
      <div className="w-full max-w-md bg-slate-50 shadow-xl rounded-2xl p-8">
        ...
        <form onSubmit={formik.handleSubmit} className="space-y-6">
          ...
          <button
            type="submit"
            className="w-full rounded-lg bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white shadow-md hover:bg-slate-800 transition"
          >
